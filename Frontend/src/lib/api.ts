...
export let fetchReviews=async():Promise<ReviewsPayload>=>{
    const res=await fetch(`${import.meta.env.VITE_API_URL}/api/reviews/hostaway`,{
        headers:authHeaders()
    })
    if(!res.ok){
...
export let toggleAproval=async(id:string)=>{
    const res=await fetch(`${import.meta.env.VITE_API_URL}/api/reviews/approve/${id}`,{
        method:'POST',
        headers:authHeaders()
    })
    if(!res.ok){
...
export async function fetchPublicReviews(): Promise<ReviewsPayload> {
    const res = await fetch('${import.meta.env.VITE_API_URL}/api/reviews/publicreviews')
